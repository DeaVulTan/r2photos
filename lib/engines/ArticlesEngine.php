<?php
include_once(Site::GetParms('tablesPath')."Mainmenu.php");
class ArticlesEngine {
   var $name; var $parms;
   function ArticlesEngine($name, $parms) { 
        $this->name = $name;
        $this->parms = $parms;
        $parmsT = array('href' => 'articles');
        if (defined("LANG")) $parmsT['lang'] = LANG;
        $menu = Mainmenu::GetRow($parmsT);
        $this->menu = $menu;
        $this->menuPath = (defined('LANG') && LANG <> 'ru' ? '/'.LANG : '').($this->menu['path'] ? $this->menu['path'] : '/');
        define("MENU_ID", ($this->menu['id'] ? $this->menu['id'] : 0));
        define("MENU_NAME", ($this->menu['name'] ? $this->menu['name'] : 'Статьи'));
        define("MENU_PATH", $this->menuPath);
        unset($menu);
   }
   
   function run() {
      if (!MENU_ID) Site::SetParms('bread-crumbs', array(MENU_NAME));
      if ($this->parms['id']) return $this->viewArticle();
      else return $this->viewArticles();
   }

   function viewArticles() {
      define("MENU_CRUMBS_LASTLINK", false);
      $page = ($this->parms['page'] ? $this->parms['page'] : 1);
      $numArticlesOnPage = Utils::GetValue('count_articles_on_page');
      $parms = array();
      if (defined("LANG")) $parms['lang'] = LANG;
      $count = Articles::GetCountRows($parms);
      if ($count > 0) {
         $numPages = ceil($count / $numArticlesOnPage);
         $articles = Articles::GetRows($parms, array('limit' => $numArticlesOnPage, 'offset' => (($page - 1) * $numArticlesOnPage)));
      }
      include(Site::GetTemplate($this->name, 'list'));
      return true;
   }

   function viewArticle() {
      define("MENU_CRUMBS_LASTLINK", true);
      $parms = array('id' => $this->parms['id']);
      if (defined("LANG")) $parms['lang'] = LANG;
      if (!$article = Articles::GetRow($parms)) return false;
      if ($this->parms['href'] <> $article['href']) return false;
      Site::SetParms('bread-crumbs', array($article['name']));
      include(Site::GetTemplate($this->name, 'one'));
      return true;
   }
}
?>