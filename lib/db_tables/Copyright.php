...
class Copyright {
   ...
   function Structure($init = false) {
      if ($init) {
         ...
         $columns = array(
            ...
            'name' => array('name' => 'Текст', 'style' => 'width="100%" align="left"', 'addstyle' => ' rows="3" cols="100" style="width: 100%;"', 'autoUpdate' => true, 'sort' => true),
         );
         if (defined('LANG')) {
      ...
      return array(
         ...
         'table'  => 'copyright',
         'tableName'  => 'Строка копирайта'
      );
   }
   function Get($id = '', $init = false) { return new Table(Copyright::Structure($init), $id); }
   function Update($rows) { $t = new Table(Copyright::Structure()); return $t->UpdateRows($rows); }
   function Save($rows) { $t = new Table(Copyright::Structure()); return $t->SaveRows($rows); }
   function Delete($rows) { $t = new Table(Copyright::Structure()); return $t->DeleteRows($rows); }
   ...
   static function GetCountRows($parms = array()) {
      $db = Site::GetDB();
      return $db->SelectValue("SELECT COUNT(*) FROM copyright".Copyright::whereString($parms));
   }

   static function GetRows($parms = array(), $limit = array(), $order = array()) {
      $db = Site::GetDB();
      return $db->SelectSet("SELECT * FROM copyright".
                             Copyright::whereString($parms).Copyright::orderString($order).
                             Copyright::limitString($limit), 'id');
   }

   static function GetRow($parms = array()) {
      $db = Site::GetDB();
      return $db->SelectRow("SELECT * FROM copyright".Copyright::whereString($parms));
   }

   static function GetDefault() {
      $db = Site::GetDB();
      return trim($db->SelectValue("SELECT name FROM copyright WHERE url_regexp='default' AND is_active=1".(defined("LANG") ? " AND lang='".mysql_real_escape_string(LANG)."'" : '')));
   }

   static function GetCopyright($parms = array()) {
      $copyright = Copyright::GetRow($parms);
      if ($copyright['id'] > 0) return trim($copyright['name']);
      return Copyright::GetDefault();
   }
}
?>
