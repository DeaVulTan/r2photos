...
class Retouches {
   ...
   function Structure($init = false) {
      ...
      return array(
         ...
         'table'  => 'retouches',
         'tableName'  => 'Примеры ретуши'
      );
   }
   function Get($id = '', $init = false) { return new Table(Retouches::Structure($init), $id); }
   function Update($rows) { $t = new Table(Retouches::Structure()); return $t->UpdateRows($rows); }
   function Save($rows) { $t = new Table(Retouches::Structure()); return $t->SaveRows($rows); }
   function Delete($rows) { $t = new Table(Retouches::Structure()); return $t->DeleteRows($rows); }
   ...
   static function GetCountRows($parms = array()) {
      $db = Site::GetDB();
      return $db->SelectValue("SELECT COUNT(*) FROM `retouches` ".Retouches::whereString($parms));
   }

   static function GetRows($parms = array(), $limit = array(), $order = array()) {
      $db = Site::GetDB();
      return $db->SelectSet("SELECT * FROM `retouches` ".
                             Retouches::whereString($parms).Retouches::orderString($order).
                             Retouches::limitString($limit), 'id');
   }

   static function GetNames($parms = array(), $limit = array(), $order = array()) {
      $db = Site::GetDB();
      return $db->SelectSet("SELECT `id`, `name` FROM `retouches` ".
                             Retouches::whereString($parms).Retouches::orderString($order).
                             Retouches::limitString($limit), 'id');
   }
   ...
   static function GetRow($parms = array()) {
      $db = Site::GetDB();
      return $db->SelectRow("SELECT * FROM `retouches` ".Retouches::whereString($parms));
   }

   static function GetIds($parms = array(), $limit = array()) {
      $db = Site::GetDB();
      return $db->SelectSet("SELECT `id` FROM `retouches` ".Retouches::whereString($parms).' ORDER BY `ord` ASC, `id` ASC '.Retouches::limitString($limit), 'id');
   }
}
?>
