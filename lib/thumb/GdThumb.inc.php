...
class GdThumb extends ThumbBase
{
    ...
    public function addWatermark($watermark) {
        if (!file_exists($watermark)) die("No file watermark.");
        
        list($widthW, $heightW) = getimagesize($watermark);
        $watermarkImage = ImageCreateFromPNG($watermark);
        $dstX = ($this->currentDimensions['width'] > $widthW ? ($this->currentDimensions['width'] - $widthW) : 0);
        $dstY = 0;
        ImageCopy($this->oldImage, $watermarkImage, $dstX, $dstY, 0, 0, $widthW, $heightW);
