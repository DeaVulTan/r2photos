<?php
class CertificatesphotosEngine extends AdminEngine {
   function CertificatesphotosEngine($name, $parms) { $this->AdminEngine($name, $parms); }
   ...
   function GetCountItems($parms = array()) {
      ...
      return Certificatesphotos::GetCountRows($parms);
   }

   function &GetItems($parms = array(), $limit = array(), $order = array()) {
      $parms['certificate_id'] = Site::GetSession($this->name."-certificate_id");
      $parms['like'] = Site::GetSession($this->name."-like");
      return Certificatesphotos::GetRows($parms, $limit, $order);
   }
   ...
   function topNav() {
      ...
      return '<a href="certificates-list.htm" title="Сертификаты">Сертификаты</a> &gt; '.( $certificate['id'] ? $certificate['name'].': ' : '' ).'Фотографии';
   }
}
?>