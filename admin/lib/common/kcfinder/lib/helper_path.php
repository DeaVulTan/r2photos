  ...
  /** Get the absolute URL path of the given one. Returns FALSE if the URL
    ...
    * @return string */
    ...
    static function rel2abs_url($path) {
        ...
        if (mb_substr($dir, 0, mb_strlen($doc_root)) != $doc_root)
            return false;

        $return = self::normalize(mb_substr($dir, mb_strlen($doc_root)) . "/$path");
    ...
    static function url2fullPath($url) {
        ...
        if (isset($_SERVER['DOCUMENT_ROOT'])) {
            ...
            if (isset($_SERVER['SCRIPT_FILENAME'])) {
                ...
                return self::normalize(mb_substr($scr_filename, 0, -mb_strlen($uri)) . "/$url");
            }
