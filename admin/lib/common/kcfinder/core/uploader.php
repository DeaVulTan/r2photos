...
class uploader {
    ...
    public function __construct() {
        ...
        // COOKIES INIT
        if (!mb_strlen($this->config['cookieDomain']))
            $this->config['cookieDomain'] = $_SERVER['HTTP_HOST'];
        if (!mb_strlen($this->config['cookiePath']))
            $this->config['cookiePath'] = "/";

        // UPLOAD FOLDER INIT
        if ($this->config['uploadURL'] == "/") {
            $this->config['uploadDir'] = mb_strlen($this->config['uploadDir'])
        ...
        if (isset($this->get['opener']) &&
            (mb_strtolower($this->get['opener']) == "tinymce") &&
            isset($this->config['_tinyMCEPath']) &&
            mb_strlen($this->config['_tinyMCEPath'])
        )
        ...
        foreach ($this->langInputNames as $key)
            if (isset($this->get[$key]) &&
                ...
                file_exists("lang/" . mb_strtolower($this->get[$key]) . ".php")
            ) {
    ...
    public function upload() {
        ...
        if ($config['disabled'] || $config['readonly']) {
            ...
            if (isset($this->get['dir']) &&
                ...
                if (mb_substr($udir, 0, mb_strlen($dir)) !== $dir)
        ...
        if (mb_strlen($message) && method_exists($this, 'errorMsg'))
    ...
    protected function checkUploadedFile() {
        ...
        $typePatt = mb_strtolower(text::clearWhitespaces($this->types[$this->type]));
    ...
    protected function validateExtension($ext, $type) {
        ...
        $exts = mb_strtolower(text::clearWhitespaces($this->config['deniedExts']));
        if (mb_strlen($exts)) {
            $exts = explode(" ", $exts);
        ...
        if (!mb_strlen($exts) || mb_substr($exts, 0, 1) == "*")
            return true;

        if (mb_substr($exts, 0, 1) == "!") {
            $exts = explode(" ", trim(mb_strtolower(mb_substr($exts, 1))));
            return !in_array($ext, $exts);
        }

        $exts = explode(" ", trim(mb_strtolower($exts)));
        return in_array($ext, $exts);
    }
    ...
    protected function makeThumb($file, $overwrite=true) {
        ...
        $thumb = mb_substr($file, mb_strlen($this->config['uploadDir']));
...
if (kc_FCKeditor)
    kc_FCKeditor(<?php echo mb_strlen($message) ? 1 : 0 ?>, '<?php echo $url ?>', '', '<?php echo $message ?>');
if (kc_Custom) {
    if (<?php echo mb_strlen($message) ?>) alert('<?php echo $message ?>');
